...
function StatCard({ label, value, icon, color = '#0F172A', sub }) {
  return (
    <div style={{ background: '#fff', borderRadius: '12px', padding: '24px', boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          <p style={{ fontSize: '0.75rem', color: '#4B5563', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: 600 }}>{label}</p>
          <p style={{ fontSize: '2rem', fontWeight: 800, color, margin: 0 }}>{value}</p>
          {sub && <p style={{ fontSize: '0.75rem', color: '#4B5563', marginTop: '4px' }}>{sub}</p>}
        </div>
...
export default async function DashboardPage() {
  ...
  return (
    <div style={{ padding: '32px', maxWidth: '1100px' }}>
      <h1 style={{ fontSize: '1.5rem', fontWeight: 800, color: '#0F172A', marginBottom: '4px' }}>Vue globale</h1>
      <p style={{ color: '#4B5563', marginBottom: '32px', fontSize: '0.88rem' }}>{today}</p>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginBottom: '40px' }}>
        <StatCard label="Utilisateurs" value={totalUsers ?? '—'} icon="👥" color="#0F172A" />
        <StatCard label="Comptes Pro+" value={proUsers} icon="⭐" color="#C8A96B" sub={totalUsers ? `${Math.round(proUsers / totalUsers * 100)}% de conversion` : ''} />
      ...
      <div style={{ background: '#fff', borderRadius: '12px', boxShadow: '0 1px 4px rgba(0,0,0,0.07)', overflow: 'hidden' }}>
        <div style={{ padding: '20px 24px', borderBottom: '1px solid #F0F0F0' }}>
          <h2 style={{ fontSize: '1rem', fontWeight: 700, color: '#0F172A', margin: 0 }}>Dernières inscriptions</h2>
        </div>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ background: '#FAFAFA' }}>
              {['Nom', 'Plan', 'Marché', 'Pays', 'Score', 'Date'].map(h => (
                <th key={h} style={{ textAlign: 'left', padding: '10px 16px', fontSize: '0.72rem', color: '#4B5563', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {(profiles || []).slice(0, 15).map((p, i) => (
              <tr key={i} style={{ borderTop: '1px solid #F5F5F5' }}>
                ...
                <td style={{ padding: '11px 16px' }}>
                  <span style={{
                    background: p.plan && !['free', null, ''].includes(p.plan) ? '#C8A96B22' : '#F0F0F0',
                    color: p.plan && !['free', null, ''].includes(p.plan) ? '#8B6914' : '#4B5563',
                ...
                <td style={{ padding: '11px 16px', fontSize: '0.85rem', color: '#4B5563' }}>{p.pays || '—'}</td>
                <td style={{ padding: '11px 16px' }}>
                  {p.audit_score
                ...
                <td style={{ padding: '11px 16px', fontSize: '0.8rem', color: '#4B5563' }}>
                  {p.created_at ? new Date(p.created_at).toLocaleDateString('fr-FR') : '—'}
                </td>
              </tr>
            ))}
            {(!profiles || profiles.length === 0) && (
              <tr><td colSpan={6} style={{ padding: '48px', textAlign: 'center', color: '#4B5563' }}>Aucun utilisateur</td></tr>
            )}
          </tbody>
        </table>
      </div>
